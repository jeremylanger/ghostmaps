...
import type { RouteInstruction, Lane } from '../types'
...
function laneArrow(direction: string): string {
  switch (direction) {
    ...
    case 'LEFT_U_TURN': return '\u21B6' // ↶
    case 'SLIGHT_RIGHT': return '\u2197' // ↗
    case 'RIGHT': return '\u2192' // →
    case 'SHARP_RIGHT': return '\u2B08' // ⬈
    case 'RIGHT_U_TURN': return '\u21B7' // ↷
    default: return '\u2191' // ↑
  }
}
...
export default function NavigationPanel() {
  ...
  const distToNext = nextInst
    ? nextInst.distance - currentInst.distance
    : 0
  ...
  return (
    <div className="navigation-panel">
      {/* Rerouting overlay */}
      {rerouting && (
        <div className="nav-rerouting">Rerouting...</div>
      )}
      ...
      <div className="nav-next-turn">
        ...
        <div className="nav-next-turn-content">
          ...
          {distToNext > 0 && (
            <div className="nav-next-turn-distance">{formatDistance(distToNext)}</div>
          )}
        </div>
      ...
      {nextInst && instructions[currentIndex + 2] && (
        <div className="nav-upcoming">
          ...
          <span className="nav-upcoming-text">Then {nextInst.message.toLowerCase()}</span>
        </div>
      )}
