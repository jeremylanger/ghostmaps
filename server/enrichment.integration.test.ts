...
describe('Place enrichment integration', () => {
  describe('Venice place briefing', () => {
    it('generates a briefing for a coffee shop', async () => {
    ...
    it('generates a briefing with minimal data', async () => {
      const briefing = await generatePlaceBriefing({
        name: 'Mystery Spot',
        category: 'restaurant',
        address: '',
    ...
    it('mentions open/closed status when provided', async () => {
      const briefing = await generatePlaceBriefing({
  ...
  describe('Place details endpoint', () => {
    it('returns enriched place from cache', async () => {
    ...
    it('returns 404 for unknown place', async () => {
      const res = await fetch('http://localhost:3001/api/places/nonexistent-id-123')
