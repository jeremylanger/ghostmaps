...
describe('EAS Review Schema', () => {
  it('should compute schema UID using encodePacked (matching Solidity)', async () => {
    ...
    const schema = 'uint8 rating, string text, string placeId, string placeName, bytes32 photoHash, int256 lat, int256 lng, uint8 qualityScore'
    ...
    const packed = concat([toUtf8Bytes(schema), getBytes(resolver), new Uint8Array([0])])
    const uid = keccak256(packed)

    expect(uid).toBe('0x968e91f0274b78a31037839b55e59b942dd1521daebf9190268137e450b7d69f')
  })
  ...
  it('should produce attest calldata that matches Interface.encodeFunctionData', async () => {
    ...
    const schema = 'uint8 rating, string text, string placeId, string placeName, bytes32 photoHash, int256 lat, int256 lng, uint8 qualityScore'
    const schemaUID = '0x968e91f0274b78a31037839b55e59b942dd1521daebf9190268137e450b7d69f'
  ...
  it('should produce register calldata that matches Interface.encodeFunctionData', async () => {
    ...
    const expected = iface.encodeFunctionData('register', [schema, ZERO_ADDRESS, false])
  ...
  it('should encode all review data fields without throwing', async () => {
    ...
    const encoded = encoder.encodeData([
      ...
      { name: 'text', value: 'Terrible. Cold food, rude staff, waited 45 minutes.', type: 'string' },
      { name: 'placeId', value: 'overture-abc-123', type: 'string' },
      { name: 'placeName', value: "McDonald's", type: 'string' },
      { name: 'photoHash', value: '0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890', type: 'bytes32' },
