...
describe('TomTom speed limits', () => {
  it('parses speed limit sections from route response', async () => {
    ...
    const result = await calculateRoute(34.05, -118.24, 34.07, -118.26, 'test-key')
  ...
  it('returns empty speedLimits when no sections in response', async () => {
    ...
    const { calculateRoute } = await import('./tomtom')
    const result = await calculateRoute(34.05, -118.24, 34.06, -118.25, 'test-key')
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ...
      json: () => Promise.resolve({
        routes: [{
          ...
          guidance: { instructions: [] },
          summary: { lengthInMeters: 100, travelTimeInSeconds: 10, trafficDelayInSeconds: 0 },
        }],
      }),
    })
...
describe('Review cache behavior', () => {
  it('review cache type structure is correct', () => {
    ...
    interface CachedReviews {
      data: { reviews: any[]; identities: Record<string, any>; summary: string }
      timestamp: number
    }
