...
describe("Venice AI integration", () => {
  describe("parseQuery", () => {
    it(
      'parses "coffee" into coffee_shop category',
      async () => {
    ...
    it(
      'parses "late night tacos" with time filter',
      async () => {
        const result = await parseQuery("late night tacos");

        expect(
          result.categories.some(
            (c) => c.includes("mexican") || c.includes("taco"),
          ),
        ).toBe(true);
    ...
    it(
      'parses "bar with outdoor seating" with attributes',
      async () => {
        const result = await parseQuery("bar with outdoor seating");

        expect(
          result.categories.some((c) => c.includes("bar") || c.includes("pub")),
        ).toBe(true);
    ...
    it(
      'parses "pizza near downtown" with location hint',
      async () => {
        const result = await parseQuery("pizza near downtown");

        expect(
          result.categories.some(
            (c) => c.includes("pizza") || c.includes("restaurant"),
          ),
        ).toBe(true);
    ...
    it(
      "returns valid structure for unknown queries",
      async () => {
  ...
  describe("rankResults", () => {
    ...
    it(
      "returns a top pick from the provided places",
      async () => {
    ...
    it(
      "returns alternatives with why_not explanations",
      async () => {
