...
export async function enrichPlace(place: Place, apiKey: string): Promise<PlaceEnrichment> {
  try {
    ...
    return {
      ...
      priceLevel: item.priceLevel ? priceLevelMap[item.priceLevel] || null : null,
      dineIn: item.dineIn ?? null,
      takeout: item.takeout ?? null,
      delivery: item.delivery ?? null,
      wheelchairAccessible: item.accessibilityOptions?.wheelchairAccessibleEntrance ?? null,
      photoUri,
    }
  } catch (err) {
