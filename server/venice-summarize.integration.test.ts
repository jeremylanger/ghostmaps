...
import { comparePlaces, summarizeReviews, verifyPhoto } from "./venice";
...
describe("Venice Review Summarization", () => {
  it("should summarize multiple reviews into a coherent paragraph", async () => {
    const summary = await summarizeReviews("Test Restaurant", [
      {
        rating: 5,
        text: "Amazing pasta, best carbonara in the city",
        qualityScore: 75,
      },
      {
        rating: 4,
        text: "Good food but slow service on weekends",
        qualityScore: 60,
      },
      {
        rating: 3,
        text: "Decent but overpriced for what you get",
        qualityScore: 45,
      },
    ]);
  ...
  it("should handle single review", async () => {
    const summary = await summarizeReviews("Solo Review Spot", [
      {
        rating: 5,
        text: "Best sushi I have ever had, the omakase was incredible",
        qualityScore: 85,
      },
    ]);
